...
Animate_RDoC <- ggplot(df, aes(SP, CC, size = PROMHBQ , color = Biotype)) + # Set up ggplot & animate specific bits:
  geom_text(aes(label = patient), nudge_y = +0.016, show.legend = FALSE) +
  stat_smooth(formula = y ~ log10(x), se = FALSE, size = 0.8, linetype="solid") +  #these are the 3 colored fitting lines
...
animate(Animate_RDoC, height = 1000, width = 1000)
